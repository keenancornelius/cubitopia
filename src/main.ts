...
class Cubitopia {
  ...
  /** Build (or rebuild) the wall mesh at a tile, connecting to all neighbor walls */
  private buildAdaptiveWallMesh(pos: HexCoord, owner: number): void {
    ...
    for (const cn of connectedNeighbors) {
      ...
      // Center the segment between pillar edge and midpoint+overlap
  ...
  private buildGateMesh(pos: HexCoord, owner: number): void {
    ...
    if (hasAdjacentGate) {
      ...
      // Build two half-towers on the sides that DON'T face another gate
      for (const gd of adjacentGateDirs) {
        const gateAngle = Math.atan2(gd.dx, gd.dz);
        // Open archway toward adjacent gate — wider passage
        const archW = 0.7;
        const archH = 1.4;
      ...
      // Passage opening perpendicular to wall line
      const passageW = 0.6;
      const passageH = 1.3;
    ...
    for (const cn of connectedDirs) {
      ...
      const segLen = halfDist + 0.15; // overlap to eliminate gaps
      const angle = Math.atan2(cn.dx, cn.dz);
      ...
      const nTile = this.currentMap.tiles.get(`${nQ},${nR}`);
